...
Agent Plugins 1.0 | 稳态发育多Agent架构 + 软件发育系统（Software Growth Studio）落地插件

> 插件目标：提供一套完整的**稳态发育架构**，用于 AI 辅助软件设计、AI 编程；采用 L0 入口 + L1~L4 渐进式技能披露，附带**稳态治理 MCP 工具**与**硬约束 hooks**，让 AI 装上即认知、按导航执行、被硬门禁约束。架构基线含**软件发育系统**：Software Genome（软件基因组）唯一真相源、一致性引擎、质量门禁、生长预算、受约束编码。
...
3. **被硬约束**：每个技能尾部有「完成态与下一步」门禁校验；MCP `check_permission`/`deviation_classify`/`consistency_check`/`quality_gate` 提供硬校验；hooks 拦截私自修改 `skills/` 目录（必须先改 `docs/` 基线）。

## 目录说明

- `plugin.json`：插件清单（Agent Plugins 1.0 规范，skills 与 mcp 自动发现）
- `mcp.json`：MCP 服务器注册（portable MCP 格式，`${PLUGIN_ROOT}` 引用）
- `mcp/stable_homeostasis_server.py`：稳态治理 MCP 服务器（Python 零依赖，stdio，14 个工具）
- `docs/`：架构原始基线文档（唯一真相源，Source of Truth；含软件发育系统基线）
- `skills/`：17 个技能（L0 入口 + L1~L4），一技能一目录
...
插件启用后自动启动，提供 14 个工具，**对齐架构四层能力 + 软件发育系统**：

| 工具 | 对应层/系统 | 对应接口/技能 | 功能 |
|------|-----------|---------------|------|
...
| `genome_relation_audit` | 软件基因组 | l3-genome-model-design | 发育谱系审计（孤儿/未验证/未文档化增长） |
| `consistency_check` | 一致性引擎 | l4-consistency-engine | 四类一致性检查（A 文档↔文档 / B 文档→代码 / C 代码→文档 / D 代码→约束） |
| `quality_gate` | 质量门禁 | l4-quality-gate | G1~G7 逐级判定（需求/设计/契约/实现/验证/一致性/发布） |
| `growth_budget` | 生长预算 | l4-quality-gate | 异常增生检测（公开类型/方法/依赖/循环依赖/复杂度） |
| `dev_context_build` | 受约束编码 | l4-dev-context | 构建 Development Context（九要素 + AI 禁止变更项） |

> 所有判定规则（偏差公式、分级阈值、白名单、权限矩阵、禁止项、阶段路由、发育谱系、门禁、预算、上下文）**直接取自 `docs/` 架构基线**，修改架构须同步更新 `mcp/stable_homeostasis_server.py` 规则数据与 `arch-version-log.md`。
...
### L3 分层设计层：按架构四层模型完成系统设计 + 软件基因组建模
...
5. `l3-genome-model-design`：软件基因组建模技能（唯一真相源 / 统一关系 / 发育谱系）

### L4 稳态运行治理层：开发流程、偏差检测、架构守护 + 软件发育闭环
...
5. `l4-consistency-engine`：一致性引擎技能（四类检查 / 一致性报告）
6. `l4-quality-gate`：质量门禁技能（G1~G7 逐级判定）
7. `l4-dev-context`：受约束编码技能（Development Context / AI 禁止变更项）

## 依赖链

```
L0 → L1-01 → L1-02 → L2-01 → L2-02 → L3-01 → L3-02 → L3-03 → L3-04 → L3-05(软件基因组) → L4-01 → L4-02 → L4-03 → L4-04 → L4-05(一致性引擎) → L4-06(质量门禁) → L4-07(受约束编码) → 闭环
```
